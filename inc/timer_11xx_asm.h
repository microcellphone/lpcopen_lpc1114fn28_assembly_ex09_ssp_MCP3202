...
#ifndef __TIMER_11XX_ASM_H_
#define __TIMER_11XX_ASM_H_
...
#define TIMER_OFFSET_IR 		0x000	/*!< Interrupt Register. The IR can be written to clear interrupts. The IR can be read to identify which of eight possible interrupt sources are pending. */
#define TIMER_OFFSET_TCR		0x004	/*!< Timer Control Register. The TCR is used to control the Timer Counter functions. The Timer Counter can be disabled or reset through the TCR. */
#define TIMER_OFFSET_TC			0x008	/*!< Timer Counter. The 32 bit TC is incremented every PR+1 cycles of PCLK. The TC is controlled through the TCR. */
#define TIMER_OFFSET_PR			0x00c	/*!< Prescale Register. The Prescale Counter (below) is equal to this value, the next clock increments the TC and clears the PC. */
#define TIMER_OFFSET_PC			0x010	/*!< Prescale Counter. The 32 bit PC is a counter which is incremented to the value stored in PR. When the value in PR is reached, the TC is incremented and the PC is cleared. The PC is observable and controllable through the bus interface. */
#define TIMER_OFFSET_MCR		0x014	/*!< Match Control Register. The MCR is used to control if an interrupt is generated and if the TC is reset when a Match occurs. */
#define TIMER_OFFSET_MR0		0x018	/*!< Match Register. MR can be enabled through the MCR to reset the TC, stop both the TC and PC, and/or generate an interrupt every time MR matches the TC. */
#define TIMER_OFFSET_MR1		0x01c	/*!< Match Register. MR can be enabled through the MCR to reset the TC, stop both the TC and PC, and/or generate an interrupt every time MR matches the TC. */
#define TIMER_OFFSET_MR2		0x020	/*!< Match Register. MR can be enabled through the MCR to reset the TC, stop both the TC and PC, and/or generate an interrupt every time MR matches the TC. */
#define TIMER_OFFSET_MR3		0x024	/*!< Match Register. MR can be enabled through the MCR to reset the TC, stop both the TC and PC, and/or generate an interrupt every time MR matches the TC. */
#define TIMER_OFFSET_CCR;		0x028	/*!< Capture Control Register. The CCR controls which edges of the capture inputs are used to load the Capture Registers and whether or not an interrupt is generated when a capture takes place. */
#define TIMER_OFFSET_CR0		0x02C	/*!< Capture Register. CR is loaded with the value of TC when there is an event on the CAPn.0 input. */
#define TIMER_OFFSET_CR1		0x030	/*!< Capture Register. CR is loaded with the value of TC when there is an event on the CAPn.0 input. */
#define TIMER_OFFSET_EMR		0x03C	/*!< External Match Register. The EMR controls the external match pins MATn.0-3 (MAT0.0-3 and MAT1.0-3 respectively). */
#define TIMER_OFFSET_CTCR		0x070	/*!< Count Control Register. The CTCR selects between Timer and Counter mode, and in Counter mode selects the signal and edge(s) for counting. */
#define TIMER_OFFSET_PWMC		0x074
 ...
 */
#define TIMER_EXTMATCH_DO_NOTHING  0	/*!< Timer match state does nothing on match pin */
#define TIMER_EXTMATCH_CLEAR       1	/*!< Timer match state sets match pin low */
#define TIMER_EXTMATCH_SET         2	/*!< Timer match state sets match pin high */
#define TIMER_EXTMATCH_TOGGLE      3	/*!< Timer match state toggles match pin */
 ...
 */
#define TIMER_CAPSRC_RISING_PCLK   0	/*!< Timer ticks on PCLK rising edge */
#define TIMER_CAPSRC_RISING_CAPN   1	/*!< Timer ticks on CAPn.x rising edge */
#define TIMER_CAPSRC_FALLING_CAPN  2	/*!< Timer ticks on CAPn.x falling edge */
#define TIMER_CAPSRC_BOTH_CAPN     3	/*!< Timer ticks on CAPn.x both edges */
